...
class GWNNTrainer(object):
    ...
    """
    def __init__(self, args, sparsifier, features, target, train_nodes=None, test_nodes=None):
        self.args = args
    ...
    def score(self):
        ...
        # score['acc1'] = correct / int(self.ncount * self.args.test_size)
        ...
        prediction = self.model(self.phi_indices,
                                   ...
                                   self.feature_indices,
                                   self.feature_values)
        score['acc'] = torchmetrics_accuracy(prediction[self.test_nodes],self.target[self.test_nodes]).detach().cpu().numpy()
        score['auc'] = torchmetrics_auc(prediction[self.test_nodes],self.target[self.test_nodes]).detach().cpu().numpy()
        logits_test = prediction[self.test_nodes].detach().cpu().numpy()
        labels = self.target[self.test_nodes].detach().cpu().numpy()
        prf_test = prf(logits_test, labels)
        score["pre"] = prf_test[0]
        score['recall'] = prf_test[1]
        score['F1'] = prf_test[2]
